...
void PBComputeMap::Print(std::ostream &s){
  for(int i=0;i<(int)node.size();++i){
    s<<"index:"<<i<<", name:"<<node[i].name<<", shape:";
    for(auto &it: node[i].shape){
      s<<it<<", ";
    }
    s<<"indexes:";
    for(auto &it: node[i].indexes){
      s<<it<<", ";
    }
    s<<std::endl;
  }
...
std::string PBComputeMap::ToJson() const{
    std::string s="[";
    s.reserve(200);
    ...
    for(int i=0;i<(int)node.size();++i){
        ...
        s+="{\"name\":\""+node[i].name+"\",\"shape\":[";
        for(int j=0;j<(int)node[i].shape.size();++j){
            if(j!=0){
                s+=",";
            }
            s+=std::to_string(node[i].shape[j]);
        }   

        s+="],\"indexes\":[";
        for(int j=0;j<(int)node[i].indexes.size();++j){
            if(j!=0){
                s+=",";
            }
            s+=std::to_string(node[i].indexes[j]);
        }   
        s+="]}";
    }
    s+="]";
...
PBComputeMap::PBComputeMap(){

}

PBComputeMap::PBComputeMap(const Array<te::Tensor>& tensors){
    // static int count=0;
    std::unordered_map<const te::OperationNode*, int> degree;
    std::unordered_map<const te::OperationNode*,
                       std::vector<const te::OperationNode*>>
        edge_set;
    ...
    while (!stack.empty()) {
        ...
        PBComputeMapNode aNode;
        this->node.push_back(aNode);
        PBComputeMapNode* pNode = &(this->node.back());
        pNode->indexes.push_back(ct);

        priority[op] = ct;
        ct++;
        visited.insert(op);

        if (auto pop = GetRef<te::Operation>(op).as<te::PlaceholderOpNode>()) {
            degree[op] = 0;
            pNode->name = op->name;
            for (auto& it : pop->shape) {
                auto aShape = it.as<IntImmNode>();
                pNode->shape.push_back(aShape->value);
            }

        } else if (auto cop =
                       GetRef<te::Operation>(op).as<te::ComputeOpNode>()) {
            ...
            pNode->name = cop->name;
            for (auto& it : cop->axis) {
                auto aShape = (it->dom->extent - it->dom->min).as<IntImmNode>();
                pNode->shape.push_back(aShape->value);
            }
    ...
    while (!stack.empty()) {
        ...
        visited.insert(op);
        if (auto cop = GetRef<te::Operation>(op).as<te::ComputeOpNode>()) {
            PBComputeMapNode* pNode =&(this->node[priority[op]]);
            const Array<te::Tensor>& input_tensors = cop->InputTensors();
            for (const auto& ten : input_tensors) {
                pNode->indexes.push_back(priority[ten->op.operator->()]);
                stack.push_back(ten->op.operator->());
            }
        }
...
Array<te::Operation> TopoSortOps(const Array<te::Tensor>& tensors) {
    ...
    while (!stack.empty()) {
        ...
        if (op->IsInstance<te::PlaceholderOpNode>()) {
            ...
            // std::cout<<cop->name<<std::endl;
            // std::cout<<cop->tag<<std::endl;
            // std::cout<<cop->attrs<<std::endl;
            // std::cout<<cop->axis<<std::endl;
            // std::cout<<cop->axis[1]->dom->extent-cop->axis[1]->dom->min
            // <<std::endl; std::cout<<cop->reduce_axis<<std::endl;
            // std::cout<<cop->body<<std::endl;
            // std::cout<<"99123"<<std::endl;
            // exit(0);
        } else {
            LOG(FATAL) << "Unsupported op " << GetRef<te::Operation>(op);
        }
...
class ReadAccessExtractor : public StmtExprVisitor {
   public:
    ...
    void VisitExpr_(const ProducerLoadNode* op) final {
        read_access[Downcast<te::Tensor>(op->producer)->op].emplace_back(
...
bool IsConstShiftEqual(const Var& var, const PrimExpr& expr) {
    ...
    if (((x + c).Match(expr) || (x - c).Match(expr) || (c + x).Match(expr) ||
         x.Match(expr)) &&
        x.Eval().same_as(var)) {
        return true;
    }
...
bool IsSimpleAccess(const te::Operation& op,
                    const std::vector<PrimExpr>& indices,
                    bool* axis_missing,
                    bool* axis_duplicated,
                    bool* same_order) {
...
void GatherVars(const PrimExpr& expr,
    ...
    for (const auto& op : node->ops_topo_order) {
        if (op->IsInstance<te::PlaceholderOpNode>()) {
            ...
            for (const auto& pair : node->read_from[op]) {
                ...
                const std::vector<std::vector<PrimExpr>>& access_list =
                    pair.second;
                const Array<PrimExpr>& output_shape = op->output_shape(0);
                const Array<PrimExpr>& producer_shape =
                    producer->output_shape(0);

                int n_common;
                for (n_common = 0; n_common < static_cast<int>(std::min(
                                                  output_shape.size(),
                                                  producer_shape.size()));
                     n_common++) {
        ...
        if (op->IsInstance<te::PlaceholderOpNode>()) {
            ...
            for (const auto& pair : node->read_from[op]) {
                const std::vector<std::vector<PrimExpr>>& access_list =
                    pair.second;
                for (const auto& access : access_list) {
                    if (!auto_scheduler::IsSimpleAccess(
                            op, access, &axis_missing, &axis_duplicated,
                            &same_order)) {
            ...
            for (const auto& pair : node->read_from[op]) {
                ...
                if (n_missing >= 2 ||
                    (n_missing >= 1 && !cop->reduce_axis.empty())) {
                    needs_multi_level_tiling = true;
                    break;
                }
            }

            // do not perform multi-level tiling on "fake reduction" with const
            // tensors
            if (op->attrs.count(
...
OperationSet AccessAnalyzer::GetConsumers(const State& state,
    ...
    collect = [this, &collect, &inlined_ops,
               &consumers](const te::Operation& op) {
...
OperationSet AccessAnalyzer::GetProducers(const State& state,
                                          const te::Operation& op) const {
    OperationSet inlined_ops;
    for (const auto& stage : state->stages) {
        if (stage->compute_at == ComputeAtKind::kInlined) {
    ...
    std::function<void(const te::Operation&)> collect;
    collect = [this, &collect, &inlined_ops,
               &producers](const te::Operation& op) {
        for (const auto& iter : operator->()->read_from.at(op)) {
            if (inlined_ops.count(iter.first)) {
                collect(iter.first);
            } else {
...
int AccessAnalyzer::GetNumCommonOuterIterator(
    ...
    traverse = [this, &traverse, &target_op, &ret, &meet](
                   const te::Operation& cur_op, int cur_num) {
        if (cur_op == target_op) {
        ...
        for (const auto& iter : operator->()->read_by.at(cur_op)) {
            traverse(
                iter.first,
                std::min(cur_num,
                         operator->()->num_common_outer_iterators.at(cur_op).at(
                             iter.first)));
        }
    };
...
bool AccessAnalyzer::ElementWiseMatch(const te::Operation& op,
    ...
    while (cur_op != target_op) {
        const AccessAnalyzerNode::OperationMap<
            std::vector<std::vector<PrimExpr>>>& map =
        ...
        for (const auto& read : reads) {
            is_simple_access = auto_scheduler::IsSimpleAccess(
                next_op, read, &axis_missing, &axis_duplicated, &same_order);
            if (!is_simple_access || axis_missing || axis_duplicated ||
                !same_order) {
                return false;
            }
        }
...
// Index calculations (e.g., the "i + j" expression in A[i + j]) are not counted
// in FLOPS.
#define VisitBinary(Node)                                  \
    double VisitExpr_(const Node* op) final {              \
        double base = 1.0;                                 \
        if ((op->a->dtype.code() != cur_type_code_) &&     \
            (op->b->dtype.code() != cur_type_code_)) {     \
            base = 0.0;                                    \
        }                                                  \
        ...
        if (stage->op->IsInstance<te::ComputeOpNode>()) {
            ...
            for (const auto& x : stage->leaf_iter_vars) {
                ICHECK(!names.count(x->var->name_hint))
                    << "Find duplicated iterator names in the compute "
                       "definition: "
                    << x->var->name_hint
...
ComputeDAG::ComputeDAG(Array<te::Tensor> tensors) {
    ...
    node->PBRoot=PBComputeMap(node->tensors);
...
ComputeDAG::ComputeDAG(const te::Schedule& sch) {
    ...
    // Initialize ops. Here we enforce the order of ops and stages are
    // consistent
    for (auto stage : sch->stages) {
        node->ops.push_back(stage->op);
    }
        ...
        if (t->op == placeholder_op_) {
            ...
            for (const auto& arg : op->indices) {
                ...
                if (const auto* int_imm = arg.as<IntImmNode>()) {
                    ...
                    axis_name =
                        AxisBaseName(CleanName(Downcast<Var>(arg)->name_hint));
            ...
            for (int i = new_names_.size() - 1; i >= 0; --i) {
                ...
                PrimExpr new_arg =
                    indexmod(indexdiv(ori_arg, div_factor), mod_factor);

                r_new_args.push_back(new_arg);
            }

            Array<PrimExpr> new_args(
                std::make_move_iterator(r_new_args.rbegin()),
...
std::string GetOrigLayout(std::set<std::string>* placeholder_axis_names,
                          const te::Operation& op,
    ...
    for (const auto& ev : extractor.read_access[placeholder_op]) {
        for (const auto& e : ev) {
            ...
            if (const auto* int_imm = e.as<IntImmNode>()) {
                ...
                axis_name =
                    AxisBaseName(CleanName(Downcast<Var>(e)->name_hint));
            }
    ...
    os.str("");
    ::tvm::relay::AutoSchedulerLayoutRewriter::global_ori_layouts_queue
        .push_back(orig_layout);
    return orig_layout;
}

std::string GetNewLayout(const State& state,
                         const int stage_id,
                         const Stage& stage,
                         const te::Operation& op,
                         const te::Tensor& placeholder,
    ...
    for (size_t i = 0; i < new_axis_names.size(); ++i) {
        ...
        if (placeholder_axis_names.count(ori_iter_name)) {
            ...
            if (iter->range.defined()) {
                ...
                // This iter is simplified by InferBound, so it must have a
                // length of one.
                extent = 1;
            }
    ...
    os.str("");
    ::tvm::relay::AutoSchedulerLayoutRewriter::global_new_layouts_queue
        .push_back(new_layout);
    return new_layout;
}
    ...
    for (size_t stage_id = 0; stage_id < state->stages.size(); stage_id++) {
        ...
        for (const auto& placeholder :
            ...
            std::string new_layout =
                GetNewLayout(state, stage_id, stage, op, placeholder,
            ...
            if (layout_rewrite ==
                ...
                new_op_to_update = te::PlaceholderOp(
                    placeholder_op->name, new_shape,
                    placeholder_op.as<te::PlaceholderOpNode>()->dtype);
            } else if (layout_rewrite ==
                       LayoutRewriteOption::InsertTransformStage) {
                ...
                const auto& layout_transform_tensor = te::compute(
                    new_shape,
                    [&new_stride, &placeholder_op, &origin_shape, &new_shape,
                     &origin_axes, &new_axes](
                        const tvm::runtime::Array<tvm::tir::Var>& indices)
                        -> tvm::PrimExpr {
                        Array<PrimExpr> access_indices;
                        for (size_t indice_index = 0;
                             indice_index < origin_shape.size();
                             indice_index++) {
                            PrimExpr temp = Integer(0);
                            for (size_t i = 0; i < new_shape.size(); i++) {
                                if (origin_axes[indice_index].compare(
                                        new_axes[i]) == 0) {
                ...
                for (size_t i = 0; i < transform_steps->size(); i++) {
                    ...
                    if (step->IsInstance<ComputeAtStepNode>()) {
                        auto compute_at_step =
                            tvm::Downcast<ComputeAtStep>(step);
                        if (compute_at_step->target_stage_id >=
                            static_cast<int>(stage_id)) {
                            dynamic_cast<ComputeAtStepNode*>(
                                compute_at_step.CopyOnWrite())
                                ->target_stage_id++;
                        }
                ...
                transform_steps->push_back(
                    AnnotationStep(stage_id, 0, IteratorAnnotation::kParallel));
            }
            ...
            for (const auto& op : p_dag->ops) {
                if (auto* pop = op.as<te::ComputeOpNode>()) {
                    ...
                    if (need_update) {
                        ...
                        new_attrs.Set("new_placeholder_layout",
                                      tvm::String(new_layout));
                        new_compute_op =
                            te::ComputeOp(pop->name, pop->tag, new_attrs,
                                          pop->axis, new_body);
                    }
                }
            }
            ...
            for (size_t i = 0; i < pops->size(); ++i) {
                ...
                if (auto* pop = original_op.as<te::ComputeOpNode>()) {
                    ...
                    if (!rmap.empty()) {
                        te::Operation new_op =
                            pop->ReplaceInputs(original_op, rmap);
...
std::pair<te::Schedule, Array<te::Tensor>> ComputeDAG::ApplySteps(
    ...
    if (layout_rewrite != LayoutRewriteOption::NoRewrite &&
        HasLayoutFreeTensors(*this) && !transform_steps.empty()) {
...
String ComputeDAG::PrintStepsAsPython(
    ...
    for (const auto& stage : stages) {
        if (stage->op->IsInstance<te::ComputeOpNode>()) {
            ...
            for (size_t i = 0; i < stage->leaf_iter_vars.size(); ++i) {
                ss << CleanName(stage->leaf_iter_vars[i]->var->name_hint,
                                op_name);
...
State ComputeDAG::InferBound(const State& state) const {
    ICHECK(state->concrete)
        << "Only concrete state can be processed to get bound info.";
    ...
    auto [sch, tensors] =
        ApplySteps(pstate->transform_steps, &stages, &stage_to_axes);
    ...
    for (size_t i = 0; i < pstate->stages.size(); ++i) {
        ...
        // the StageToAxesMap is used to find the corresponding IterVar in TVM
        // schedule result
        for (size_t j = 0; j < stage->iters.size(); ++j) {
            ...
            if (find_res != bounds.end()) {
                new_iters.push_back(Iterator(iter->name, (*find_res).second,
                                             iter->iter_kind, iter->annotation,
                                             &iter->orig_iters));
            } else {
        ...
        pstate->stages.Set(i, Stage(stage->op, stage->op_type, new_iters,
...
Array<State> ComputeDAG::InferBound(const Array<State>& states) const {
    ...
    support::parallel_for(
        0, states.size(), [this, &states, &out_states](int i) {
            try {
                out_states.Set(i, (states[i].defined())
                                      ? this->InferBound(states[i])
                                      : states[i]);
            } catch (Error& e) {
...
ComputeDAG ComputeDAG::ReplayAndGetDAG(
    const Array<Step>& transform_steps) const {
...
TVM_STATIC_IR_FUNCTOR(ReprPrinter, vtable)
    .set_dispatch<AccessAnalyzerNode>([](const ObjectRef& ref, ReprPrinter* p) {
        ...
        for (const auto& op : node->ops_topo_order) {
            ...
            p->stream << "is_strictly_inlinable:\t"
                      << node->is_strictly_inlineable.at(op) << "\t";
            ...
            for (size_t j = 0; j < node->ops_topo_order.size(); ++j) {
                ...
                if (ana.ElementWiseMatch(node->ops_topo_order[i],
                    ...
                    p->stream << node->ops_topo_order[i]->name << " -> "
                              << node->ops_topo_order[j]->name << std::endl;
                }
            }
        }
        p->stream << Chars('=', 50) << std::endl;
        ...
        for (const auto& src_pair : node->num_common_outer_iterators) {
            for (const auto& dst_pair : src_pair.second) {
                p->stream << src_pair.first->name << " " << dst_pair.first->name
                          << " " << dst_pair.second << std::endl;
            }
        }
        p->stream << Chars('=', 50) << std::endl;
    });
...
Array<PrimExpr> GetShapeFromRewrittenLayout(String rewritten_layout,
    ...
    CHECK_EQ(ct, extracted_names.size())
        << "The number or names of axes do not match";
...
TVM_REGISTER_GLOBAL("auto_scheduler.RewriteIndexForNewLayout")
    .set_body_typed([](const te::Operation& placeholder_op,
                       const std::string& new_layout,
                       const PrimExpr& body) {
        IndexRewriter index_rewriter(placeholder_op, new_layout);
...
TVM_REGISTER_GLOBAL("auto_scheduler.RewriteTensorShape")
    .set_body_typed([](te::Tensor tensor, Array<PrimExpr> new_shape) -> void {
        ...
        te::PlaceholderOpNode* op = const_cast<te::PlaceholderOpNode*>(
            tensor->op.as<te::PlaceholderOpNode>());
